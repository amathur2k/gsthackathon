...
# Define the massive neural network model with geometric progression
class MassiveNet(nn.Module):
    def __init__(self, input_size, num_layers, initial_neurons, num_classes):
        ...
        for i in range(1, num_layers):
            neurons = max(int(initial_neurons * (0.75 ** i)), 16)  # Ensure at least 16 neurons
            self.layers.append(nn.Linear(int(initial_neurons * (0.75 ** (i-1))), neurons))
            self.layers.append(nn.ReLU())
            self.layers.append(nn.BatchNorm1d(neurons))
            self.layers.append(nn.Dropout(0.3))

        # Output layer
        self.layers.append(nn.Linear(max(int(initial_neurons * (0.75 ** (num_layers-1))), 16), num_classes))
...
num_layers = 8
initial_neurons = 1024  # Start with 32,768 neurons
num_classes = 2

# Initialize the massive model
massive_model = MassiveNet(input_size, num_layers, initial_neurons, num_classes).to(device)

# Calculate and print the total number of trainable parameters
total_params = sum(p.numel() for p in massive_model.parameters() if p.requires_grad)
print(f"Total trainable parameters: {total_params}")

# Initialize the loss function and optimizer

class_weights = torch.tensor([1.01, 3.01]).to('cuda')
...
print("Model saved as 'massive_model.pth'")
